	...
	$(document)
			.ready(
					function() {
						...
						body = $("body");
						nav_closed_width = 50;
						nav_open = body.hasClass("main-nav-opened")
								|| nav.width() > nav_closed_width;
						$("#main-nav .dropdown-collapse")
								.on(
										"click",
										function(e) {
											var link, list;
											...
											if (list.is(":visible")) {
												if (body
														.hasClass("main-nav-closed")
														&& link.parents("li").length === 1) {
													false;
												} else {
													link.removeClass("in");
													list
						...
						// 页面索引
						pageControl.pageMap = {
							userManage : '../html/user/user.html',
							distributorManage : '../html/distributor/distributor.html'
						};

						// 默认首先加载信息看板页面
						pageControl.load("userManage");

						return $(document).bind(
								"nav-open",
								function(event, params) {
									body.addClass("main-nav-opened")
											.removeClass("main-nav-closed");
